...
pub fn main() !void {
    ...
    if (tune) {
        ...
        if (nnue_stats and use_nnue) {
            ...
            try nnue.print_debug_stats(&out_writer.interface);
        }
    } else if (perft_test) {
        ...
        var stdout_buffer: [4096]u8 = undefined;
        var out_writer = std.fs.File.stdout().writer(&stdout_buffer);
        _ = try test_suite.run_all_tests(&out_writer.interface);
        ...
        var stdout_buffer: [4096]u8 = undefined;
        var out_writer = std.fs.File.stdout().writer(&stdout_buffer);
        try nnue.microbench(allocator, nnue_microbench_iterations, &out_writer.interface);
    } else if (do_datagen) {
