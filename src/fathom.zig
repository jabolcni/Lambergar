...
pub const TB_MAX_MOVES: usize = fathom.TB_MAX_MOVES;
...
pub fn init_tablebases(allocator: std.mem.Allocator, path: ?[]const u8) !void {
    ...
    if (path == null) {
        //std.debug.print("info string No tablebase path provided\n", .{});
        return;
    }
    ...
    compat.accessAbsolute(p) catch |err| {
        _ = printout(uci.stdout, "info string Failed to access tablebase directory: {s}, error: {}\n", .{ p, err });
        return;
    };

    const null_terminated_path = try allocator.dupeZ(u8, p);
    defer allocator.free(null_terminated_path);
...
inline fn tb_position_looks_legal(white_kings: u64, black_kings: u64) bool {
    if (bb.pop_count(white_kings) != 1 or bb.pop_count(black_kings) != 1) {
        return false;
    }
    const wk_sq = bb.get_ls1b_index(white_kings);
    const wk_attacks = att.KING_ATTACKS[wk_sq];
    return (wk_attacks & black_kings) == 0;
}
...
pub fn probeWDL(curr_pos: *Position, depth: i8) c_uint {
    ...
    if (!tb_position_looks_legal(white_kings, black_kings)) {
        return fathom.TB_RESULT_FAILED;
    }
    ...
    // Fathom expects true for white to move, false for black.
    const turn: bool = (curr_pos.side_to_play == Color.White);
    const castling: c_uint = @intCast(curr_pos.history[curr_pos.game_ply].castling);
    if (castling != 0) {
        return fathom.TB_RESULT_FAILED;
    }
    ...
    const result = fathom.tb_probe_wdl_shim(
        white,
        black,
...
pub fn probeRoot(curr_pos: *Position, results: []Move, depth: i8) struct { result: c_uint, move_count: u32 } {
    ...
    var raw_results: [TB_MAX_MOVES]c_uint = undefined;
    const result = fathom.tb_probe_root_shim(
        white,
        black,
        white_kings | black_kings,
        white_queens | black_queens,
    ...
    for (raw_results[0..@min(results.len, TB_MAX_MOVES)], 0..) |raw_move, i| {
