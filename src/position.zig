...
pub const PawnHashEntry = struct {
    key: u64 = 0,
    valid: bool = false,
    mg_score: i32 = 0,
    eg_score: i32 = 0,
    white_pawn_attacks: u64 = 0,
    black_pawn_attacks: u64 = 0,
    white_west_attacks: u64 = 0,
    white_east_attacks: u64 = 0,
    black_west_attacks: u64 = 0,
    black_east_attacks: u64 = 0,
    white_passed_front: u64 = 0,
    black_passed_front: u64 = 0,
    white_passers: u64 = 0,
    black_passers: u64 = 0,
};
...
pub const Position = struct {
    ...
    pawn_entry: PawnHashEntry = PawnHashEntry{},
    pawn_entry_valid: bool = false,
    pawn_entry_stack: [2048]PawnHashEntry = undefined,
    pawn_entry_valid_stack: [2048]bool = undefined,
    ...
    pub fn new() Position {
        ...
        pos.pawn_entry = PawnHashEntry{};
        pos.pawn_entry_valid = false;
        pos.pawn_entry_stack[0] = pos.pawn_entry;
        pos.pawn_entry_valid_stack[0] = pos.pawn_entry_valid;
        pos.eval.eval_mg = 0;
        pos.eval.eval_eg = 0;
    ...
    pub fn copy(from: Position) Position {
        return Position{
            ...
            .pawn_entry = from.pawn_entry,
            .pawn_entry_valid = from.pawn_entry_valid,
            .pawn_entry_stack = from.pawn_entry_stack,
            .pawn_entry_valid_stack = from.pawn_entry_valid_stack,
            .eval = from.eval,
    ...
    pub fn computeMoveGenContext(self: *Position, comptime Us: Color) MoveGenContext {
        ...
        const our_king_sq = Square.fromU6(our_king);
        const orth_line_mask = bb.MASK_RANK[our_king_sq.rank_of().toU3()] | bb.MASK_FILE[our_king_sq.file_of().toU3()];
        const diag_line_mask = bb.MASK_DIAGONAL[our_king_sq.diagonal_of()] | bb.MASK_ANTI_DIAGONAL[our_king_sq.anti_diagonal_of()];

        var b1: u64 = 0;

        // Reuse the cached full enemy attack map, then patch in only the king-xray
        // slider attacks that differ when our king is removed from occupancy.
        var danger: u64 = self.attacked_squares(Them);
        var checkers: u64 = 0;
        var pinned: u64 = 0;
        ...
        var orth_candidates = their_orth_sliders & orth_line_mask;
        while (orth_candidates != 0) {
            const s = bb.pop_lsb(&orth_candidates);
            const between = attacks.SQUARES_BETWEEN_BB[our_king][s];
            if ((between & them_bb) != 0) continue;

            danger |= attacks.piece_attacks(s, all_bb ^ SQUARE_BB[our_king], PieceType.Rook);
            b1 = between & us_bb;
        ...
        var diag_candidates = their_diag_sliders & diag_line_mask;
        while (diag_candidates != 0) {
            const s = bb.pop_lsb(&diag_candidates);
            const between = attacks.SQUARES_BETWEEN_BB[our_king][s];
            if ((between & them_bb) != 0) continue;

            danger |= attacks.piece_attacks(s, all_bb ^ SQUARE_BB[our_king], PieceType.Bishop);
            b1 = between & us_bb;
    ...
    inline fn invalidate_pawn_entry(self: *Position) void {
        self.pawn_entry_valid = false;
        self.pawn_entry_valid_stack[self.game_ply] = false;
    }
    ...
    pub inline fn move_piece(self: *Position, from: u6, to: u6) void {
        ...
        if (from_pc.type_of() == PieceType.Pawn or to_pc.type_of() == PieceType.Pawn) {
            self.invalidate_pawn_entry();
        }
    }
    ...
    pub inline fn attacked_squares(self: *Position, comptime C: Color) u64 {
        const idx = C.toU4();
        const curr_hist = &self.history[self.game_ply];
        if (curr_hist.attacked_valid[idx]) {
            return curr_hist.attacked_by[idx];
        }

        const occ = self.all_pieces(Color.White) | self.all_pieces(Color.Black);
        var danger = if (self.pawn_entry_valid)
            if (C == Color.White) self.pawn_entry.white_pawn_attacks else self.pawn_entry.black_pawn_attacks
        else
            attacks.pawn_attacks_from_bitboard(self.bitboard_of_pt(C, PieceType.Pawn), C);

        const king_bb = self.bitboard_of_pt(C, PieceType.King);
        if (king_bb != 0) {
            danger |= attacks.piece_attacks(bb.get_ls1b_index(king_bb), occ, PieceType.King);
        }

        var knights = self.bitboard_of_pt(C, PieceType.Knight);
        while (knights != 0) {
            danger |= attacks.piece_attacks(bb.pop_lsb(&knights), occ, PieceType.Knight);
        }

        var diag_sliders = self.diagonal_sliders(C);
        while (diag_sliders != 0) {
            danger |= attacks.piece_attacks(bb.pop_lsb(&diag_sliders), occ, PieceType.Bishop);
        }

        var orth_sliders = self.orthogonal_sliders(C);
        while (orth_sliders != 0) {
            danger |= attacks.piece_attacks(bb.pop_lsb(&orth_sliders), occ, PieceType.Rook);
        }

        curr_hist.attacked_by[idx] = danger;
        curr_hist.attacked_valid[idx] = true;
        return danger;
    }
    ...
    pub inline fn is_insufficient_material(self: *Position) bool {
        ...
        const white_minor_count = white_knight_count + white_bishop_count;
        const black_minor_count = black_knight_count + black_bishop_count;

        // K+N vs K or K+B vs K
        if ((white_minor_count == 1 and black_minor_count == 0) or
            (white_minor_count == 0 and black_minor_count == 1))
        {
            return true;
        }

        // K+B vs K+B with bishops restricted to the same square color.
        if (white_knight_count == 0 and black_knight_count == 0 and
            white_bishop_count == 1 and black_bishop_count == 1)
        {
    ...
    pub fn play(self: *Position, m: Move, comptime C: Color) void {
        ...
        self.game_ply += 1;
        const prev_hist = &self.history[self.game_ply - 1];
        const curr_hist = &self.history[self.game_ply];
        curr_hist.* = UndoInfo.copy(prev_hist.*);
        curr_hist.move = m;
        self.pawn_entry_stack[self.game_ply] = self.pawn_entry;
        self.pawn_entry_valid_stack[self.game_ply] = self.pawn_entry_valid;

        const update_entry = SQUARE_BB[m.to] | SQUARE_BB[m.from];
        curr_hist.entry |= update_entry;

        self.delta.reset();
        curr_hist.accumulator.computed_accumulation = false;
        curr_hist.accumulator.computed_score = false;

        if (curr_hist.castling > 0) {
            var new_rights: u4 = curr_hist.castling;
            const to_clear: u4 = self.castle_rights_clear_by_sq[m.from] | self.castle_rights_clear_by_sq[m.to];
            if (to_clear != 0) {
                new_rights &= ~to_clear;
                if (new_rights != curr_hist.castling) {
                    curr_hist.castling = new_rights;
                    self.hash ^= zobrist.castling_keys[prev_hist.castling] ^ zobrist.castling_keys[curr_hist.castling];
                }
            }
        }

        var epsq = prev_hist.epsq;
        ...
        switch (m.flags) {
            ...
            MoveFlags.DOUBLE_PUSH => {
                ...
                curr_hist.epsq = Square.fromU6(@as(u6, @intCast(@as(i8, @intCast(m.from)) + Direction.NORTH.relative_dir(C).toI8())));
                self.hash ^= zobrist.enpassant_keys[curr_hist.epsq.file_of().toU3()];
            },
            MoveFlags.OO => {
    ...
    pub fn undo(self: *Position, m: Move, comptime C: Color) void {
        ...
        switch (m.flags) {
            ...
            MoveFlags.PC_KNIGHT, MoveFlags.PC_BISHOP, MoveFlags.PC_ROOK, MoveFlags.PC_QUEEN => {
                self.remove_piece(m.to);
                self.put_piece(Piece.new(C, PieceType.Pawn), m.from);
                self.put_piece(curr_hist.captured, m.to);
            },
            MoveFlags.CAPTURE => {
                self.move_piece_quiet(m.to, m.from);
                self.put_piece(curr_hist.captured, m.to);
        ...
        self.hash ^= zobrist.side_key;
        self.game_ply -= 1;
        const prev_hist = &self.history[self.game_ply];
        ...
        if (curr_hist.castling != prev_hist.castling) {
            self.hash ^= zobrist.castling_keys[curr_hist.castling] ^ zobrist.castling_keys[prev_hist.castling];
        }

        self.pawn_entry = self.pawn_entry_stack[self.game_ply];
        self.pawn_entry_valid = self.pawn_entry_valid_stack[self.game_ply];
    }
    ...
    pub fn is_legal_quiet_no_evasion_from_ctx(self: *Position, comptime Us: Color, ctx: MoveGenContext, move: Move) bool {
        if (ctx.check_count != 0 or !move.is_quiet()) return false;
        if (move.flags == MoveFlags.OO or move.flags == MoveFlags.OOO) return false;

        const pc = self.board[move.from];
        if (pc == Piece.NO_PIECE or pc.color() != Us) return false;
        if (self.board[move.to] != Piece.NO_PIECE) return false;

        const from_bb = SQUARE_BB[move.from];
        const to_bb = SQUARE_BB[move.to];
        if ((ctx.us_bb & to_bb) != 0) return false;

        const pt = pc.type_of();
        if (pt != PieceType.King and (ctx.pinned & from_bb) != 0 and (attacks.LINE[ctx.our_king][move.from] & to_bb) == 0) {
            return false;
        }

        return switch (pt) {
            PieceType.Pawn => blk: {
                if (move.flags != MoveFlags.QUIET and move.flags != MoveFlags.DOUBLE_PUSH) break :blk false;
                if (relative_rank_of_u6(move.from, Us) == Rank.RANK7.toU6()) break :blk false;

                const from_i: i16 = @intCast(move.from);
                const to_i: i16 = @intCast(move.to);
                const north: i16 = @intCast(Direction.NORTH.relative_dir(Us).toI8());
                const one_i = from_i + north;
                if (one_i < 0 or one_i >= 64) break :blk false;

                if (move.flags == MoveFlags.QUIET) {
                    break :blk to_i == one_i;
                }

                const two_i = one_i + north;
                if (two_i < 0 or two_i >= 64) break :blk false;
                if (relative_rank_of_u6(move.from, Us) != Rank.RANK2.toU6()) break :blk false;

                const one: u6 = @intCast(one_i);
                break :blk to_i == two_i and self.board[one] == Piece.NO_PIECE;
            },
            PieceType.Knight => move.flags == MoveFlags.QUIET and (ctx.pinned & from_bb) == 0 and (attacks.piece_attacks(move.from, ctx.all_bb, PieceType.Knight) & to_bb) != 0,
            PieceType.Bishop => move.flags == MoveFlags.QUIET and (attacks.piece_attacks(move.from, ctx.all_bb, PieceType.Bishop) & to_bb) != 0,
            PieceType.Rook => move.flags == MoveFlags.QUIET and (attacks.piece_attacks(move.from, ctx.all_bb, PieceType.Rook) & to_bb) != 0,
            PieceType.Queen => move.flags == MoveFlags.QUIET and (((attacks.piece_attacks(move.from, ctx.all_bb, PieceType.Bishop) | attacks.piece_attacks(move.from, ctx.all_bb, PieceType.Rook)) & to_bb) != 0),
            PieceType.King => move.flags == MoveFlags.QUIET and (attacks.piece_attacks(move.from, ctx.all_bb, PieceType.King) & to_bb) != 0 and (ctx.danger & to_bb) == 0,
            PieceType.NoType => false,
        };
    }
    ...
    fn generate_castling_moves_standard(self: *Position, comptime Us: Color, ctx: MoveGenContext, list: *MoveList) void { // classical O-O and O-O-O only
        ...
        const king_piece = Piece.new(Us, PieceType.King);
        const rook_piece = Piece.new(Us, PieceType.Rook);

        // Kingside O-O
        if (have_oo and (self.board[std_ks] == king_piece) and (self.board[std_rk] == rook_piece)) {
        ...
        // Queenside O-O-O
        if (have_ooo and (self.board[std_ks] == king_piece) and (self.board[std_rq] == rook_piece)) {
    ...
    fn generate_castling_moves_frc(self: *Position, comptime Us: Color, ctx: MoveGenContext, list: *MoveList) void { // FRC/Chess960 castling
        ...
        // Kingside (O-O)
        if (have_oo and self.castle_rook_k_start[ci] != Square.NO_SQUARE) {
        ...
        // Queenside (O-O-O)
        if (have_ooo and self.castle_rook_q_start[ci] != Square.NO_SQUARE) {
            const ks = ctx.our_king;
    ...
    pub fn generate_legals_from_ctx(self: *Position, comptime Us: Color, ctx: MoveGenContext, list: *MoveList) void {
    ...
    pub fn generate_legals(self: *Position, comptime Us: Color, list: *MoveList) void {
        ...
        self.generate_legals_from_ctx(Us, ctx, list);
    }  

    // Generates all legal moves in the current position.
    pub fn generate_noisy_legals_from_ctx(self: *Position, comptime Us: Color, ctx: MoveGenContext, list: *MoveList) void {
        if (ctx.check_count > 0) {
            self.generate_evasions(Us, ctx, list);
            return;
        }
    ...
    pub fn generate_noisy_legals(self: *Position, comptime Us: Color, list: *MoveList) void {
        const ctx = self.computeMoveGenContext(Us);
        self.generate_noisy_legals_from_ctx(Us, ctx, list);
    }          
...
test "insufficient material excludes minor-piece mate positions" {
    attacks.initialise_all_databases();
    zobrist.initialise_zobrist_keys();

    const drawn_fens = [_][]const u8{
        "8/7k/8/8/8/8/8/K7 w - - 0 1",
        "8/7k/8/8/8/8/8/KB6 w - - 0 1",
        "8/7k/8/8/8/8/8/KN6 w - - 0 1",
    ...
    for (drawn_fens) |fen| {
        var pos = Position.new();
        try pos.set(fen);
        try std.testing.expect(pos.is_insufficient_material());
    }

    const not_drawn_fens = [_][]const u8{
        "6nk/5N2/6K1/8/8/8/8/8 b - - 0 1",
        "6bk/5N2/6K1/8/8/8/8/8 b - - 0 1",
        "6nk/5B2/6K1/8/8/8/8/8 b - - 0 1",
        "7k/8/6b1/8/8/8/8/K1B5 w - - 0 1",
    };

    for (not_drawn_fens) |fen| {
        var pos = Position.new();
        try pos.set(fen);
        try std.testing.expect(!pos.is_insufficient_material());
    }

    var mate_pos = Position.new();
    try mate_pos.set("6nk/5N2/6K1/8/8/8/8/8 b - - 0 1");
    var moves: MoveList = .{};
    mate_pos.generate_legals(Color.Black, &moves);
    try std.testing.expect(mate_pos.in_check(Color.Black));
...
test "test_movegen_speed" {
    ...
    const Timer = compat_timer.Timer;
