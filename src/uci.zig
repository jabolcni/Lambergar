...
var stdin_reader: compat.FileReader = undefined;
var stdout_writer: compat.FileWriter = undefined;
...
pub fn printout(writer: *std.Io.Writer, comptime str: []const u8, args: anytype) void {
    writer.print(str, args) catch std.process.exit(0);
    writer.flush() catch std.process.exit(0);
}
...
pub fn init_all(allocator: std.mem.Allocator) !void {
    ...
    stdin_reader = compat.stdinReader(&buffer);
    stdout_writer = compat.stdoutWriter(&stdout_buffer);

    stdin = &stdin_reader.interface;
    stdout = &stdout_writer.interface;
}

// Loads the embedded NNUE network if NNUE is enabled.
pub fn init_nnue_if_needed() !void {
...
pub fn uci_loop(allocator: std.mem.Allocator) !void {
    ...
    try init_nnue_if_needed();
    if (debug) std.debug.print("NNUE loaded = {}\n", .{nnue.engine_loaded_net});

    try tt.TT.init(HASH_SIZE_DEFAULT + 1);
    defer tt.TT.deinit();
    ...
    mainloop: while (true) {
        ...
        // Consume the trailing newline as well; `takeDelimiterExclusive`
        // leaves it buffered, which traps the loop on the same `\n`.
        const input_full = (try compat.takeDelimiter(stdin, '\n')) orelse break :mainloop;
        if (input_full.len == 0) continue :mainloop;
        const input = std.mem.trim(u8, input_full, "\r");
        ...
        if (std.mem.eql(u8, command, "uci")) {
            if (debug) std.debug.print("uci command\n", .{});
            printout(stdout, "id name Lambergar 1.5\n", .{});
            ...
            // UCI: respond to `isready` immediately, even while searching.
            printout(stdout, "readyok\n", .{});
        } else if (std.mem.eql(u8, command, "debug")) {
            const arg = words.next() orelse continue;
            ...
            if (std.mem.eql(u8, arg, "name")) {
                ...
                if (std.mem.eql(u8, arg, "Hash")) {
                    ...
                    arg = words.next().?;
                    if (std.mem.eql(u8, arg, "value")) {
                        num_threads = std.math.clamp(try usize_from_str(words.next() orelse continue), 1, MAX_THREADS);
                        @atomicStore(bool, &thinkers[0].stop, true, .seq_cst);
            ...
            for (0..list.count) |i| {
                ...
                if ((move.flags == position.MoveFlags.OO or move.flags == position.MoveFlags.OOO) and is_chess960() and pos[0].is_chess960) {
                    ...
                    if (rook_sq != position.Square.NO_SQUARE) {
                        ...
                        printout(stdout, "{s}{s}{s}", .{ k_from, r_from, position.PROM_TYPESTR[move.flags.toU4()] });
                        continue;
                    }
                }
            ...
            // Usage: datagen games N [depth D] [plies P] [random FIRST NEXT] [debug] [strict] [skipnoisy] [filename NAME.bin] [eval NNUE|HCE] [format bin40|binhce]
                ...
                if (std.mem.eql(u8, arg, "games")) {
                    ...
                    if (words.next()) |mode| {
                        if (std.ascii.eqlIgnoreCase(mode, "hce")) {
                            cfg.eval_mode = datagen.EvalMode.HCE;
                        } else {
                            cfg.eval_mode = datagen.EvalMode.NNUE;
                        }
                    }
                } else if (std.mem.eql(u8, arg, "format")) {
                    if (words.next()) |fmt| {
                        if (std.ascii.eqlIgnoreCase(fmt, "binhce")) {
                            cfg.format = datagen.BinFormat.binhce;
                        } else {
                            cfg.format = datagen.BinFormat.bin40;
                        }
                    }
                } else if (std.mem.eql(u8, arg, "dither")) {
                    if (words.next()) |v| cfg.dither_depth = u32_from_str(v) catch cfg.dither_depth;
                } else if (std.mem.eql(u8, arg, "min_nodes")) {
                    if (words.next()) |v| cfg.min_nodes = u64_from_str(v) catch cfg.min_nodes;
                } else if (std.mem.eql(u8, arg, "phase_prob_opening")) {
                    if (words.next()) |v| cfg.phase_prob_opening = std.fmt.parseFloat(f32, v) catch cfg.phase_prob_opening;
                } else if (std.mem.eql(u8, arg, "phase_prob_early_middlegame")) {
                    if (words.next()) |v| cfg.phase_prob_early_middlegame = std.fmt.parseFloat(f32, v) catch cfg.phase_prob_early_middlegame;
                } else if (std.mem.eql(u8, arg, "phase_prob_middlegame")) {
                    if (words.next()) |v| cfg.phase_prob_middlegame = std.fmt.parseFloat(f32, v) catch cfg.phase_prob_middlegame;
                } else if (std.mem.eql(u8, arg, "phase_prob_late_middlegame")) {
                    if (words.next()) |v| cfg.phase_prob_late_middlegame = std.fmt.parseFloat(f32, v) catch cfg.phase_prob_late_middlegame;
                } else if (std.mem.eql(u8, arg, "phase_prob_endgame")) {
                    if (words.next()) |v| cfg.phase_prob_endgame = std.fmt.parseFloat(f32, v) catch cfg.phase_prob_endgame;
                }
            }

            // Normalize filename extension based on format
            var final_name = cfg.filename;
            const ext = if (cfg.format == datagen.BinFormat.binhce) ".binhce" else ".bin";
            if (!std.mem.endsWith(u8, final_name, ext)) {
                final_name = std.fmt.allocPrint(allocator, "{s}{s}", .{ cfg.filename, ext }) catch cfg.filename;
            }
            nnue.engine_using_nnue = cfg.eval_mode == datagen.EvalMode.NNUE;
            printout(stdout, "info string datagen start games={} depth={} plies={} bin={s} eval={s} format={s}\n", .{ cfg.games, cfg.best_depth, cfg.max_plies, final_name, @tagName(cfg.eval_mode), @tagName(cfg.format) });
        ...
        } else if (std.mem.eql(u8, command, "hcefeatures")) {
            var feats = tuner.Tuner.new();
            feats.init();
            _ = pos[0].eval.clean_eval(&pos[0], &feats);
            ...
            printout(stdout, "info string hce_features {s}\n", .{out.items});
        } else if (std.mem.eql(u8, command, "hcefeatures_sfen32")) {
            if (words.next()) |hex| {
                if (hex.len != 64) {
                    printout(stdout, "info string hce_features_sfen32 error expected 64 hex chars\n", .{});
                    continue;
                }
                ...
                var i: usize = 0;
                while (i < 32) : (i += 1) {
                    const hi = std.fmt.parseInt(u8, hex[i * 2 .. i * 2 + 2], 16) catch {
                        ok = false;
                        break;
                    };
                    raw[i] = hi;
                ...
                if (!ok) {
                    printout(stdout, "info string hce_features_sfen32 error invalid hex\n", .{});
                    continue;
                }
                binw.unpack_sfen32(&raw, &pos[0]);
                var feats = tuner.Tuner.new();
                feats.init();
                _ = pos[0].eval.clean_eval(&pos[0], &feats);
                ...
                printout(stdout, "info string hce_features_sfen32 error missing hex\n", .{});
            }
        ...
        } else if (command.len >= 3 and std.mem.eql(u8, command[0..3], "see")) {
            const move_str = if (command.len > 4) command[4..] else continue;
            ...
            if (has_castling_rights) {
                ...
                if (wdl_result == fathom.TB_RESULT_FAILED) {
                    ...
                    // Probe for best move
                    const TB_MAX_MOVES: usize = fathom.TB_MAX_MOVES;
...
pub fn bench(allocator: std.mem.Allocator, depth: u32, use_nnue: bool) !void {
    ...
    nnue.engine_using_nnue = use_nnue;
    try init_nnue_if_needed();
    ...
    var nodes: u64 = 0;
    var timer = compat_timer.Timer.start() catch |err| {
